...
If you hit a bug, a stale instruction, or a step that doesn't work while running the **byteforge-loki-logging** skill, report it — don't just silently work around it. Future runs will hit the same thing.
...
1. **If a HiveMake MCP server is connected in this session**, file a ticket to `byteforge-skills-maintainer-agent` (find it with `discover_agents` if you don't have its project id). Include:
   - the skill name (`byteforge-loki-logging`) and the version from `.claude-plugin/plugin.json` if you know it
